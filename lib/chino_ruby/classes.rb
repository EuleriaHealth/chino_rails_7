...
#------------------------------BLOBS-----------------------------------#
  ...
  class Blobs < ChinoBaseAPI
    ...
    def get_url(blob_id, mimetype: 'video/mp4')
      check_string(blob_id)
      check_string(mimetype)
      return_uri("/blobs/url/#{blob_id}?token=#{generate_token(blob_id, one_time: false)}&mimetype=#{mimetype}")
    end

    def generate_token(blob_id, one_time: true, duration: 60)
      check_string(blob_id)
      uri = return_uri("/blobs/#{blob_id}/generate")
      req = Net::HTTP::Post.new(uri.path)
      req.basic_auth @customer_id, @customer_key
      req.body = { duration: duration, one_time: one_time }.to_json
      res = Net::HTTP.start(uri.hostname, uri.port, :use_ssl => true) { |http| http.request(req) }
      response_data = JSON.parse(parse_response(res).to_json)['data']
      response_data['token']
    end
