...
class SDKTest < Test::Unit::TestCase
   ...
   def test_users
       ...
       usr = @client.users.update_user_partial(usr.user_id, nil, "new_username")
       assert_equal(usr.username, "new_username")
       assert_not_equal(usr.user_id, "")
   ...
   def test_blobs
       ...
       # 'path' was formerly 'test/testfiles', but the SDK docs say that tests should be run from inside folder 'test'. Let's handle both cases
       if not Dir.pwd.end_with?("test")
           path = "test/" + path
       end
